...
def main():
	...
	args.inputFile = r"C:\sampledata\kmall\0020_20231018_233527_FC_CL.kmall"	
...
def process(filename):
	'''demonstrate how to access the kmall file and extract the data we want'''
	...
	#to cmpute a point cloud we need to define a projection for the data.  We can do this by reading the first position record in the file
	...
	while r.moreData():
		...
		if typeofdatagram == '#MRZ':
			...
			rawbytes = datagram.loadbytes()
			datagram.decode()
			# datagram.decode(rawbytes)
			# datagram.read()
			# print(datagram.pingCnt) #, datagram.beams[datagram.centrebeam].depth)
...
class RANGEDEPTH:
	...
	def decode(self, rawbytes=""):
		'''this decode requires the raw bytes to be passed in. '''

		rec_fmt = kmallreader.EMdgmHeader_def
		rec_len1 = struct.calcsize(rec_fmt)
		rec_unpack = struct.Struct(rec_fmt).unpack
		if len(rawbytes) == 0:
			self.fileptr.seek(self.offset, 0)# move the file pointer to the start of the record so we can read from disc
			s = rec_unpack(self.fileptr.read(rec_len1))
		else:
			# slice out the part we need...
			s = struct.unpack_from(rec_fmt, rawbytes)
			# s = rec_unpack(rawbytes[0:rec_len1])
		decodeheader(s, self)

		rec_fmt = kmallreader.EMdgmMpartition_def
		rec_len2 = struct.calcsize(rec_fmt)
		rec_unpack = struct.Struct(rec_fmt).unpack

		if len(rawbytes) == 0:
			s = rec_unpack(self.fileptr.read(rec_len2))
		else:
			# slice out the part we need...
			s = struct.unpack_from(rec_fmt, rawbytes[rec_len1:])
		...
		rec_len3 = struct.calcsize(rec_fmt)
		rec_unpack = struct.Struct(rec_fmt).unpack

		if len(rawbytes) == 0:
			s = rec_unpack(self.fileptr.read(rec_len3))
		else:
			# slice out the part we need...
			s = struct.unpack_from(rec_fmt, rawbytes[rec_len1+rec_len2:])
		...
		rec_len4 = struct.calcsize(rec_fmt)
		rec_unpack = struct.Struct(rec_fmt).unpack

		if len(rawbytes) == 0:
			s = rec_unpack(self.fileptr.read(rec_len4))
		else:
			# slice out the part we need...
			s = struct.unpack_from(rec_fmt, rawbytes[rec_len1+rec_len2+rec_len3:])
		...
		for i in range(self.numTxSectors):
			...
			rec_len5 = struct.calcsize(rec_fmt)
			rec_unpack = struct.Struct(rec_fmt).unpack

			if len(rawbytes) == 0:
				s = rec_unpack(self.fileptr.read(rec_len5))
			else:
				# slice out the part we need...
				s = struct.unpack_from(rec_fmt, rawbytes[rec_len1+rec_len2+rec_len3+rec_len4+(i*rec_len5):])
		...
		rec_len6 = struct.calcsize(rec_fmt)
		rec_unpack = struct.Struct(rec_fmt).unpack

		if len(rawbytes) == 0:
			s = rec_unpack(self.fileptr.read(rec_len6))
		else:
			# slice out the part we need...
			s = struct.unpack_from(rec_fmt, rawbytes[rec_len1+rec_len2+rec_len3+rec_len4+(self.numTxSectors*rec_len5):])

		self.numBytesRxInfo				= s[0]
		self.numSoundingsMaxMain		= s[1] # number of beams to process
		self.centrebeam 				= int(self.numSoundingsMaxMain/2)
		...
		rec_len7 = 0
		for i in range(self.numExtraDetections): #pkpk improve performance here
			EMdgmMRZ_extraDetClassInfo_def = "HbB"
			rec_fmt = EMdgmMRZ_extraDetClassInfo_def
			rec_len7 = struct.calcsize(rec_fmt)
			rec_unpack = struct.Struct(rec_fmt).unpack

			if len(rawbytes) == 0:
				s = rec_unpack(self.fileptr.read(rec_len7))
			else:
				# slice out the part we need...
				s = struct.unpack_from(rec_fmt, rawbytes[rec_len1+rec_len2+rec_len3+rec_len4+(self.numTxSectors*rec_len5)+rec_len6:])
		...
		rec_len8 = struct.calcsize(rec_fmt)
		rec_unpack = struct.Struct(rec_fmt).unpack
		...
		for i in range(self.numSoundingsMaxMain):
			if len(rawbytes) == 0:
				...
				s = rec_unpack(self.fileptr.read(rec_len8))
			else:
				beambyteoffset = 0
		...
		rec_len9 = struct.calcsize(rec_fmt)
		rec_unpack = struct.Struct(rec_fmt).unpack

		if len(rawbytes) == 0:
			s = rec_unpack(self.fileptr.read(rec_len9))
		else:
			# slice out the part we need...
			s = struct.unpack_from(rec_fmt, rawbytes[rec_len1+rec_len2+rec_len3+rec_len4+(self.numTxSectors*rec_len5)+rec_len6+(self.numExtraDetections*rec_len7)+(self.numSoundingsMaxMain*rec_len8):])
